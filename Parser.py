...
def parse_email(filepath):
    ...
    if not shipping_number:
        for pattern in shipping_number_patterns:
            ...
            if shipping_number:
             shipping_number = shipping_number.group(1)
    if not shipping_number:
    ...
    expected_delivery_patterns = ['expected delivery', 'estimated delivery', 'delivery date', 'shipping status']
    ...
    order_number = None
    order_number_patterns = ['order number', 'order id', 'order number:']
